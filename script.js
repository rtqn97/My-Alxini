...
const slides = [
  ...
  { title: "Signature", text: `You were, you are, and you will always be deeply loved. 
I hope this message helps you understand my heart and my intentions. 
I am here patiently, hoping for understanding, and wishing only the best for you — always.

With Love and Care — zeroZERO`, bg: "#FFF8E7" }
];
...
function createPetals() {
  ...
  for(let i=0;i<30;i++){
    ...
    // Continuous gentle horizontal drift
...
function showSlide(index){
  ...
  typeWriter(slideText, slide.text, 30, () => {
    // LAST SLIDE BUTTON LOGIC
    if(currentSlide === slides.length - 1){
      nextBtn.textContent = "Repeat";
      nextBtn.classList.remove('hidden');

      // Remove previous End button if exists
      const existingEnd = document.getElementById('end-btn');
      ...
      endBtn.addEventListener('click', () => { window.close(); });
      slidesContainer.appendChild(endBtn);
    } else {
...
nextBtn.addEventListener('click', () => {
  if(currentSlide === slides.length - 1){
    currentSlide = 0; // Repeat from beginning
...
startBtn.addEventListener('click', () => {
  ...
  // Music play and volume fade
  bgMusic.volume = 0.6;
  bgMusic.play();
  let vol = 0.6;
  const volInterval = setInterval(() => {
    vol += 0.01;
    if(vol >= 0.9){ vol = 0.9; clearInterval(volInterval); }
    bgMusic.volume = vol;
  }, 30);
